...
sortBed | 
mergeBed -i - |
subtractBed -nonamecheck -a stdin -b $bedtools_dir/hg38_exons.bed | sortBed > $bedtools_dir/hg38_introns.bed

cat $index_dir_path/gencode.v27.primary_assembly.annotation.gtf | 
awk 'BEGIN{OFS="\t";} $3=="gene" {print $1,$4-1,$5}' | 
sortBed | 
mergeBed -i - |
complementBed -i stdin -g $index_dir_path/hg38.genome | sortBed > $bedtools_dir/hg38_intergenic.bed
