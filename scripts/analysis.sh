...
for pipeline in $work_dir/*; do
    ...
    for paper_dir in $pipeline/final_output/*; do
        ...
        for gtf_file in $paper_dir/*.gtf; do 
            ...
            gff_output_prefix=$(echo "$(basename $gtf_file)"| sed s/stringtie_merged.gtf/.stats/)
            ...
            intersectBed -a $work_dir/bedtools/hg38_exons.bed -b $bedtools_input > $work_dir/$pipeline_name/final_output/$paper_name/$bedtools_output"exons.bed"
            intersectBed -a $work_dir/bedtools/hg38_introns.bed -b $bedtools_input > $work_dir/$pipeline_name/final_output/$paper_name/$bedtools_output"introns.bed"
            intersectBed -a $work_dir/bedtools/hg38_intergenic.bed -b $bedtools_input > $work_dir/$pipeline_name/final_output/$paper_name/$bedtools_output"intergenic.bed"
        done
        fi
    done
    fi
